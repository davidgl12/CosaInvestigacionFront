...
export function deleteTweet(idUser, mensaje) {
  getUserTweetsApi(idUser, 1)
    .then(tweets => {
      console.log({tweets});
      console.log({mensaje});
      const tweet = tweets.find(t => t.mensaje.includes(mensaje));
      console.log({tweet});
      if(tweet === undefined){
        console.log({tweet}, 'watafok')
        return 'No se encontró un tweet con ese mensaje';
      }
      
      const url = `${API_HOST}/eliminarTweet?id=${tweet.id}`;
      ...
      return fetch(url, params)
        .then((response) => {
          return 'Se eliminó tweet'
        })
        .catch((err) => {
          return 'No se pudo eliminar el tweet';
        });
