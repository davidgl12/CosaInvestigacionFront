...
import Bot from "../../components/BotChat/Bot"
...
function MyUser(props) {
  ...
  useEffect(() => {
    getUserTweetsApi(loggedUser._id, 1)
      .then((response) => {
        setTweets(response);
      })
      .catch(() => {
  ...
  const moreData = () => {
    ...
    getUserTweetsApi(loggedUser._id, pageTemp).then((response) => {
      if (!response) {
        setLoadingTweets(0);
      } else {
